...
export default function GamesCatalogScreen({ onLogout, API_URL }) {
  ...
  const removeFromCart = (gameId, all = false) => {
    setCartItems(prev => {
      const existing = prev.find(item => item.id === gameId);
              ...
              {purchaseSuccess ? (
                ...
                <View style={styles.totalContainer}>
                  <Text style={[styles.totalText, { color: '#ff4b4b', fontSize: 16 }]}>
                    ¿Seguro que deseas eliminar todos los juegos?
                  </Text>
